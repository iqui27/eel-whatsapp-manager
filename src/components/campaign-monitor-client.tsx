  ...
  useEffect(() => {
    ...
    const fetchSegmentName = async () => {
      try {
        ...
        if (!cancelled) setSegmentName(data[0]?.name ?? 'Segmento removido');
      } catch {
        if (!cancelled) setSegmentName('Segmento removido');
      }
    };
    ...
    <SidebarLayout currentPage="campanhas" pageTitle="Monitor de Campanha">
      <div className="p-6 space-y-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={fetchCampaign} className="gap-1.5">
          ...
          <Card>
            <CardContent className="pt-5 pb-4 space-y-3">
              ...
              <div className="relative h-3 w-full rounded-full bg-muted overflow-hidden">
                <div
                  className={cn('h-full rounded-full transition-all duration-500', isSending ? 'bg-amber-500' : 'bg-green-500')}
